...
Aurafine은 보컬 트랙과 MR(반주) 트랙을 입력받아 보컬을 보정하고, 레벨을 맞춘 뒤 마스터링된 WAV 파일을 만드는 Python CLI 프로젝트입니다. 보컬 대신 드럼/베이스/일렉기타/통기타/피아노 중 2개 이상을 넣어 레벨만 맞추는 `stems` 모드도 있습니다.
...
`mix` 모드에서는 `--mr` 옵션이 필수입니다. `voice` 모드에서는 `--mr`를 전달하지 않습니다. `stems` 모드에서는 `--drum --bass --electric-guitar --acoustic-guitar --piano` 중 **2개 이상**을 넣으면 되고, 다섯 개를 다 넣을 필요는 없습니다. `--vocal`/`--mr`/`--reverb`는 무시됩니다.

`stems` 모드는 **커맨드라인에 가장 먼저 적은 트랙**을 기준(0dB)으로 삼고, 나머지 트랙을 정해진 목표 레벨에 맞춥니다. 목표 레벨은 드럼을 0dB로 뒀을 때 베이스 -2dB, 일렉기타 -5dB, 통기타 -6dB, 피아노 -5dB이며, 어느 트랙이 기준이 되든 두 트랙 사이의 상대적인 차이는 항상 같게 유지됩니다. 이 값은 장르 공통 기본값이라 실제 곡으로 들어보며 조정이 필요할 수 있습니다.
...
M1 파이프라인과 CLI가 구현되어 있고, 합성 테스트 음원으로 세 모드 모두 끝까지 실행되는 것을 확인했습니다 (믹스/스템 결과: -14 LUFS, 피크 -1dBFS 이내). 디에서는 치찰음이 커질 때만 반응하는 다이내믹 방식이고, 보컬/MR 밸런싱은 1초 구간 RMS를 부드럽게 보간하며 보컬을 MR보다 +3dB 위에 얹습니다. `stems` 모드는 같은 밸런싱 로직을 재사용해, 커맨드라인에 먼저 입력한 트랙을 기준으로 나머지 트랙을 정해진 목표 레벨에 맞춥니다 (2개 이상이면 드럼 없이도 동작). 자동 테스트는 아직 구성되지 않았습니다.
